...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-slate-950 p-3 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl text-xs font-semibold text-slate-800 dark:text-slate-200">
...
export function ComparisonChart({ data, title }: Props) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-slate-900 p-6 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
      ...
      <ResponsiveContainer width="100%" height="100%" minHeight={250}>
        <BarChart data={data} margin={{ top: 25, right: 0, left: -20, bottom: 0 }} barGap={8}>
          ...
          <Bar dataKey="T1" fill="currentColor" radius={[4, 4, 0, 0]} name="T1 (Inicial)" maxBarSize={60} className="text-slate-400 dark:text-slate-600">
          ...
          <Bar dataKey="T3" fill="currentColor" radius={[4, 4, 0, 0]} name="T3 (Final)" maxBarSize={60} className="text-blue-600 dark:text-blue-500">
            <LabelList dataKey="T3" position="top" fill="currentColor" fontSize={12} fontWeight="bold" className="text-blue-600 dark:text-blue-500" />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
    </div>
