...
const CustomTooltip = ({ active, payload }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-[#001022] p-3 border border-slate-200 dark:border-[#002D72]/30 rounded-xl shadow-xl text-xs font-semibold text-slate-800 dark:text-slate-200">
...
export function ComparisonChart({ data, title }: Props) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-white dark:bg-[#001833] p-6 border border-slate-200 dark:border-[#002D72]/30 rounded-xl shadow-sm">
      ...
      <ResponsiveContainer width="100%" height="100%" minHeight={250}>
        <BarChart data={data} margin={{ top: 25, right: 0, left: -20, bottom: 0 }} barGap={8}>
          ...
          <Bar dataKey="T1" fill="currentColor" radius={[4, 4, 0, 0]} name="T1 (Inicial)" maxBarSize={60} className="text-slate-300 dark:text-[#002D72]/60">
