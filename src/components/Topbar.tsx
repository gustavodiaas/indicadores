...
export function Topbar({ onExportWord }: Props) {
  return (
    ...
    <div className="relative group inline-block print:hidden z-50">
      <button className="flex items-center gap-2 px-3 py-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-700 bg-transparent rounded-full hover:bg-slate-200/50 transition-colors">
        <ShieldAlert className="w-4 h-4" />
        Privacidade e LGPD
      </button>
      
      {/* Painel Flutuante (Aparece no Hover) */}
      <div className="absolute top-full right-0 mt-2 w-[340px] p-5 bg-slate-900 text-slate-300 text-xs rounded-xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 border border-slate-700 text-justify cursor-default">
