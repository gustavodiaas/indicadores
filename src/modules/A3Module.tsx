...
import { Download, LayoutTemplate, Plus, Trash2, AlertTriangle, Pencil, ChevronDown, Calendar as CalendarIcon, ChevronLeft, ChevronRight } from "lucide-react";
...
// Campo de digitação básico customizado local para o A3
    ...
    <div className="w-full text-left">
      ...
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
        <DropdownMenuTrigger asChild>
          <button type="button" className="w-full h-12 px-4 rounded-xl bg-slate-50 dark:bg-slate-950 text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center justify-between outline-none hover:bg-slate-100 dark:hover:bg-slate-900 transition-all focus:ring-2 focus:ring-[#0057FF] border border-transparent text-left">
        ...
        <DropdownMenuContent align="start" className="p-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-xl z-[150] w-72">
          <div className="flex items-center justify-between mb-4">
            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (currentMonth === 0) { setCurrentMonth(11); setCurrentYear(y => y - 1); } else { setCurrentMonth(m => m - 1); } }} className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-400 transition-all">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <span className="text-xs font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wider">{months[currentMonth]} {currentYear}</span>
            <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); if (currentMonth === 11) { setCurrentMonth(0); setCurrentYear(y => y + 1); } else { setCurrentMonth(m => m + 1); } }} className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg text-slate-600 dark:text-slate-400 transition-all">
...
const TextAreaBlock = ({ title, value, maxChars, onChangeField }: { title: string, value: string, maxChars: number, onChangeField: (v: string) => void }) => {
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800/80 overflow-hidden shadow-sm shrink-0 min-h-[150px] flex-1 relative">
      ...
      <textarea
        className="flex-1 w-full p-3 pb-8 text-xs text-slate-600 dark:text-slate-200 outline-none resize-none bg-transparent leading-relaxed"
        placeholder="Descreva de forma resumida e direta..."
        value={value || ""}
        onChange={(e) => onChangeField(e.target.value)}
      />
      <div className={`absolute bottom-2 right-3 text-[10px] font-bold ${isOverLimit ? 'text-rose-500' : 'text-slate-400 dark:text-slate-500'}`}>
...
export function A3Module({ data, onChange }: Props) {
  ...
  const confirmDelete = () => {
    ...
    if (deleteConfirm.type === 'acao') {
      ...
      onChange({ indicadores: listaIndicadores.filter(i => i !== deleteConfirm.id) });
    }
  ...
  return (
    <div className="flex flex-col gap-4 h-full pb-36 animate-in fade-in duration-500 overflow-y-auto pr-2">
      <Dialog open={deleteConfirm.open} onOpenChange={(o) => setDeleteConfirm(prev => ({ ...prev, open: o }))}>
        <DialogContent className="bg-white dark:bg-slate-900 rounded-2xl border-none shadow-2xl p-8 max-w-sm mx-auto">
          <div className="flex flex-col items-center text-center">
            ...
            <DialogHeader>
              <DialogTitle className="text-xl font-bold text-slate-800 dark:text-slate-100">Confirmar Exclusão</DialogTitle>
      ...
      <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-xl p-3 flex items-center gap-3 shrink-0">
      ...
      <div className="flex flex-col gap-4 bg-slate-50/50 dark:bg-transparent p-6 rounded-2xl border border-slate-100 dark:border-slate-800/40">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-100 dark:border-slate-800/80 shadow-sm shrink-0 items-end">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col shrink-0">
              ...
              <div className="p-3 flex flex-col gap-2 bg-slate-50/50 dark:bg-slate-950">
                {listaPlanoAcao.map((a) => (
                  <div key={a.id} className="grid grid-cols-12 gap-2 items-center">
                    ...
                    <div className="col-span-5"><input type="text" className="w-full text-xs p-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:border-[#0057FF] focus:ring-1 focus:ring-[#0057FF]" value={a.oque} onChange={e => updatePlanoAcao(a.id, "oque", e.target.value)} placeholder="O que fazer?" /></div>
                    <div className="col-span-3"><input type="text" className="w-full text-xs p-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:border-[#0057FF] focus:ring-1 focus:ring-[#0057FF]" value={a.quem} onChange={e => updatePlanoAcao(a.id, "quem", e.target.value)} placeholder="Quem?" /></div>
                    <div className="col-span-2"><input type="text" className="w-full text-xs p-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:border-[#0057FF] focus:ring-1 focus:ring-[#0057FF]" value={a.prazo} onChange={e => updatePlanoAcao(a.id, "prazo", e.target.value)} placeholder="Prazo" /></div>
              ...
              <div className="p-3 flex flex-col gap-2 bg-slate-50/50 dark:bg-slate-950">
                {listaIndicadores.map((i) => (
                  <div key={i.id} className="grid grid-cols-12 gap-2 items-center">
                    <div className="col-span-1 flex justify-center"><Pencil className="w-3 h-3 text-slate-300 dark:text-slate-600" /></div>
                    <div className="col-span-5"><input type="text" className="w-full text-xs p-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:border-[#0057FF] focus:ring-1 focus:ring-[#0057FF]" value={i.indicador} onChange={e => updateIndicador(i.id, "indicador", e.target.value)} placeholder="Indicador" /></div>
                    <div className="col-span-3"><input type="text" className="w-full text-xs p-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 outline-none focus:border-[#0057FF] focus:ring-1 focus:ring-[#0057FF]" value={i.meta} onChange={e => updateIndicador(i.id, "meta", e.target.value)} placeholder="Meta" /></div>
                    <div className="col-span-2">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <button className="w-full h-8 px-2 rounded-md border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-[10px] font-medium text-slate-700 dark:text-slate-300 flex items-center justify-between outline-none hover:bg-slate-50 dark:hover:bg-slate-800 transition-all focus:ring-1 focus:ring-[#0057FF] focus:border-[#0057FF]">
                        ...
                        <DropdownMenuContent className="w-32 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-1.5 rounded-xl shadow-xl z-[150]">
                          <DropdownMenuItem onClick={() => updateIndicador(i.id, "status", "")} className={`w-full text-left text-[11px] font-bold py-2 px-2.5 rounded-lg cursor-pointer transition-all ${!i.status ? "bg-[#0057FF] text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>Limpar</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => updateIndicador(i.id, "status", "No Prazo")} className={`w-full text-left text-[11px] font-bold py-2 px-2.5 rounded-lg cursor-pointer transition-all ${i.status === "No Prazo" ? "bg-[#0057FF] text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>No Prazo</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => updateIndicador(i.id, "status", "Atrasado")} className={`w-full text-left text-[11px] font-bold py-2 px-2.5 rounded-lg cursor-pointer transition-all ${i.status === "Atrasado" ? "bg-[#0057FF] text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>Atrasado</DropdownMenuItem>
                          <DropdownMenuItem onClick={() => updateIndicador(i.id, "status", "Concluído")} className={`w-full text-left text-[11px] font-bold py-2 px-2.5 rounded-lg cursor-pointer transition-all ${i.status === "Concluído" ? "bg-[#0057FF] text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"}`}>Concluído</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                    <div className="col-span-1 text-center"><button onClick={() => setDeleteConfirm({ open: true, type: 'indicador', id: i.id })} className="text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 p-1.5 rounded-lg transition-colors"><Trash2 className="w-3.5 h-3.5" /></button></div>
                  </div>
                ))}
              </div>
            </div>
