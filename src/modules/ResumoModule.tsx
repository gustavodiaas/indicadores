...
import { type ResumoData, type AppState, type Acao5W2H, calcProdutividade, calcPayback } from "@/store/useAppStore";
...
export function ResumoModule({ data, state, onChange }: Props) {
  ...
  // CASAMENTO PERFEITO: O texto reflete exatamente o seu HTML original
  ...
  return (
    <div className="flex gap-8 h-full">
      {/* Lado Esquerdo: O Formulário do seu HTML */}
      <div className="w-[50%] flex flex-col gap-6 overflow-y-auto pr-4 pb-12">
        <h3 className="font-bold text-slate-800 border-b pb-2 text-lg uppercase tracking-tight">Dados do Projeto</h3>
        ...
        <div className="grid grid-cols-1 gap-4 pt-4 border-t">
          <InputField label="Processo Produtivo" value={data.processos} onChange={v => onChange({ processos: v })} />
          <div className="grid grid-cols-2 gap-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InputField label="Setor das Oportunidades" value={data.oportunidades} onChange={v => onChange({ oportunidades: v })} />
        ...
        <div className="pt-4 border-t space-y-4">
          <h4 className="text-sm font-bold text-slate-700 uppercase">Plano de Ação</h4>
          <div className="flex gap-2">
            <div className="flex-1"><InputField label="Ação realizada" value={newAcao.what || ""} onChange={v => setNewAcao({ what: v })} /></div>
            <button onClick={() => { if(newAcao.what) { onChange({ acoes: [...data.acoes, { id: Date.now().toString(), what: newAcao.what, why: "", where: "", when: "", who: "", how: "", howMuch: "" }] }); setNewAcao({}); } }} className="h-10 mt-6 px-4 bg-blue-600 text-white rounded-md font-bold text-xs uppercase shadow-lg shadow-blue-200">Add</button>
          </div>
      ...
      {/* Lado Direito: A "Conversa" que o HTML puxava */}
      <div className="w-[50%] flex flex-col gap-6 overflow-y-auto pb-12">
        
        {/* Bloco de Descrição */}
        <div className="relative bg-white p-6 rounded-2xl border-2 border-slate-100 shadow-sm transition-all hover:border-blue-200">
        ...
        <div className="relative bg-[#0f172a] p-8 rounded-2xl shadow-xl border border-slate-800">
          ...
          <div className="text-[13px] text-slate-300 leading-relaxed text-justify space-y-6">
            ...
            <p className="text-[11px] text-slate-500 italic leading-relaxed pt-4 border-t border-slate-800">
